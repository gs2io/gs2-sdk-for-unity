...
{
	[Preserve]
	public class QueryIssueStampSheetLogResult
	{
        /** スタンプシート発行ログのリスト */
        public List<IssueStampSheetLog> items { set; get; }
    	...
    	[Preserve]
        public static QueryIssueStampSheetLogResult FromDict(JsonData data)
        {
            return new QueryIssueStampSheetLogResult {
                items = data.Keys.Contains("items") && data["items"] != null ? data["items"].Cast<JsonData>().Select(value =>
                    {
                        return Gs2.Gs2Log.Model.IssueStampSheetLog.FromDict(value);
                    }
                ).ToList() : null,
