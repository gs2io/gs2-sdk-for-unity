...
{
	[Preserve]
	public class CountExecuteStampSheetLogResult
	{
        /** スタンプシート実行ログ集計のリスト */
        public List<ExecuteStampSheetLogCount> items { set; get; }
    	...
    	[Preserve]
        public static CountExecuteStampSheetLogResult FromDict(JsonData data)
        {
            return new CountExecuteStampSheetLogResult {
                items = data.Keys.Contains("items") && data["items"] != null ? data["items"].Cast<JsonData>().Select(value =>
                    {
                        return Gs2.Gs2Log.Model.ExecuteStampSheetLogCount.FromDict(value);
                    }
                ).ToList() : null,
