...
{
	[Preserve]
	public class CountExecuteStampTaskLogResult
	{
        /** スタンプタスク実行ログ集計のリスト */
        public List<ExecuteStampTaskLogCount> items { set; get; }
    	...
    	[Preserve]
        public static CountExecuteStampTaskLogResult FromDict(JsonData data)
        {
            return new CountExecuteStampTaskLogResult {
                items = data.Keys.Contains("items") && data["items"] != null ? data["items"].Cast<JsonData>().Select(value =>
                    {
                        return Gs2.Gs2Log.Model.ExecuteStampTaskLogCount.FromDict(value);
                    }
                ).ToList() : null,
