...
{
	[Preserve]
	public class DescribeOutputsResult
	{
        /** アウトプットのリスト */
        public List<Output> items { set; get; }
    	...
    	[Preserve]
        public static DescribeOutputsResult FromDict(JsonData data)
        {
            return new DescribeOutputsResult {
                items = data.Keys.Contains("items") && data["items"] != null ? data["items"].Cast<JsonData>().Select(value =>
                    {
                        return Gs2.Gs2Deploy.Model.Output.FromDict(value);
                    }
                ).ToList() : null,
