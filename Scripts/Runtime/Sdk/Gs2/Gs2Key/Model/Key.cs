...
namespace Gs2.Gs2Key.Model
{
    ...
    public static string GetKeyNameFromGrn(
        string grn
    )
    {
        var match = Regex.Match(grn, "grn:gs2:(?<region>.*):(?<ownerId>.*):key:(?<namespaceName>.*):key:(?<keyName>.*)");
        if (!match.Groups["keyName"].Success)
        {
            return null;
        }
    ...
    )
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            if (secret == null && secret == other.secret)
            {
                // null and null
            }
            else
            {
                diff += secret.CompareTo(other.secret);
            }
