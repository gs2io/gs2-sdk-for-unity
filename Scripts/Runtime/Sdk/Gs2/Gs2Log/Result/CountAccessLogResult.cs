...
{
	[Preserve]
	public class CountAccessLogResult
	{
        /** アクセスログ集計のリスト */
        public List<AccessLogCount> items { set; get; }
    	...
    	[Preserve]
        public static CountAccessLogResult FromDict(JsonData data)
        {
            return new CountAccessLogResult {
                items = data.Keys.Contains("items") && data["items"] != null ? data["items"].Cast<JsonData>().Select(value =>
                    {
                        return Gs2.Gs2Log.Model.AccessLogCount.FromDict(value);
                    }
                ).ToList() : null,
