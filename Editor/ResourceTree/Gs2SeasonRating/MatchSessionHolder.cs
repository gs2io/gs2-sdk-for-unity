...
{
    public sealed class MatchSessionHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public MatchSessionHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "MatchSession";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2SeasonRatingRestClient(session).DescribeMatchSessionsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new MatchSession(++id, this._parent, v)
            ).ToList<TreeViewItem>();
