...
{
    public sealed class SeasonModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public SeasonModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "SeasonModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2SeasonRatingRestClient(session).DescribeSeasonModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new SeasonModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
