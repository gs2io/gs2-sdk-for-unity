...
namespace Gs2.Editor.ResourceTree.Gs2SkillTree.Editor
{
    ...
    {
        public static void OnGUI(Gs2.Gs2SkillTree.Model.Namespace item) {
            ...
            if (item.ReleaseScript == null) {
                EditorGUILayout.TextField("ReleaseScript", "");
            }
            else {
                EditorGUILayout.LabelField("ReleaseScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.ReleaseScript);
                EditorGUI.indentLevel--;
            }
            if (item.RestrainScript == null) {
                EditorGUILayout.TextField("RestrainScript", "");
            }
            else {
                EditorGUILayout.LabelField("RestrainScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.RestrainScript);
                EditorGUI.indentLevel--;
            }
