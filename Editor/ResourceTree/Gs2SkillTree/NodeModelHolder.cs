...
{
    public sealed class NodeModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public NodeModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "NodeModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2SkillTreeRestClient(session).DescribeNodeModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new NodeModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
