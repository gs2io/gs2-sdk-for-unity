...
namespace Gs2.Editor.ResourceTree.Gs2Guard.Editor
{
    ...
    {
        public static void OnGUI(Gs2.Gs2Guard.Model.Namespace item) {
            ...
            if (item.BlockingPolicy == null) {
                EditorGUILayout.TextField("BlockingPolicy", "");
            }
            else {
                EditorGUILayout.LabelField("BlockingPolicy");
                EditorGUI.indentLevel++;
                BlockingPolicyModelEditorExt.OnGUI(item.BlockingPolicy);
                EditorGUI.indentLevel--;
            }
