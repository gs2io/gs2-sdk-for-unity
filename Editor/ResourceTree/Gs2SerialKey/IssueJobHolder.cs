...
{
    public sealed class IssueJobHolder : AbstractTreeViewItem
    {
        private CampaignModel _parent;

        public IssueJobHolder(
                int id,
                CampaignModel parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "IssueJob";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2SerialKeyRestClient(session).DescribeIssueJobsFuture(
                new DescribeIssueJobsRequest()
                        .WithNamespaceName(this._parent.NamespaceName)
                        .WithCampaignModelName(this._parent.CampaignModelName)
            );
            yield return future;
            ...
            this.children = future.Result.Items.Select(
                v => new IssueJob(++id, this._parent, v)
            ).ToList<TreeViewItem>();
