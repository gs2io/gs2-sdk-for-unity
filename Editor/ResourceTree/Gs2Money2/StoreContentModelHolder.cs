...
{
    public sealed class StoreContentModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public StoreContentModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "StoreContentModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Money2RestClient(session).DescribeStoreContentModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new StoreContentModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
