...
{
    public sealed class UnusedBalanceHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public UnusedBalanceHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "UnusedBalance";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Money2RestClient(session).DescribeUnusedBalancesFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new UnusedBalance(++id, this._parent, v)
            ).ToList<TreeViewItem>();
