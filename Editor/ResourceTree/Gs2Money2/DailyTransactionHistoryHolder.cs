...
{
    public sealed class DailyTransactionHistoryHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public DailyTransactionHistoryHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "DailyTransactionHistory";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Money2RestClient(session).DescribeDailyTransactionHistoriesFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new DailyTransactionHistory(++id, this._parent, v)
            ).ToList<TreeViewItem>();
