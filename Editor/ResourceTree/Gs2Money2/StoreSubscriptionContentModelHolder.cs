...
{
    public sealed class StoreSubscriptionContentModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public StoreSubscriptionContentModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "StoreSubscriptionContentModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Money2RestClient(session).DescribeStoreSubscriptionContentModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new StoreSubscriptionContentModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
