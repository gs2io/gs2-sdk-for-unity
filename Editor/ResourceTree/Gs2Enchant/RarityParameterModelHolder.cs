...
{
    public sealed class RarityParameterModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public RarityParameterModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "RarityParameterModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2EnchantRestClient(session).DescribeRarityParameterModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new RarityParameterModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
