...
{
    public sealed class BalanceParameterModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public BalanceParameterModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "BalanceParameterModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2EnchantRestClient(session).DescribeBalanceParameterModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new BalanceParameterModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
