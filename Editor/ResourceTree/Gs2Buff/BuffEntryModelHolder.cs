...
{
    public sealed class BuffEntryModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public BuffEntryModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "BuffEntryModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2BuffRestClient(session).DescribeBuffEntryModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new BuffEntryModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
