...
{
    public sealed class SimpleInventoryModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public SimpleInventoryModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "SimpleInventoryModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InventoryRestClient(session).DescribeSimpleInventoryModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new SimpleInventoryModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
