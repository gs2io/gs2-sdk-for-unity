...
{
    public sealed class ItemModelHolder : AbstractTreeViewItem
    {
        private InventoryModel _parent;

        public ItemModelHolder(
                int id,
                InventoryModel parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "ItemModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InventoryRestClient(session).DescribeItemModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new ItemModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
