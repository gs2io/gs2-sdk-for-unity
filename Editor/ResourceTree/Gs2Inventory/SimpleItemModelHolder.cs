...
{
    public sealed class SimpleItemModelHolder : AbstractTreeViewItem
    {
        private SimpleInventoryModel _parent;

        public SimpleItemModelHolder(
                int id,
                SimpleInventoryModel parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "SimpleItemModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InventoryRestClient(session).DescribeSimpleItemModelsFuture(
                new DescribeSimpleItemModelsRequest()
                        .WithNamespaceName(this._parent.NamespaceName)
                        .WithInventoryName(this._parent.InventoryName)
            );
            yield return future;
            ...
            this.children = future.Result.Items.Select(
                v => new SimpleItemModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
