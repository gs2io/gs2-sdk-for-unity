...
{
    public sealed class BigInventoryModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public BigInventoryModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "BigInventoryModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InventoryRestClient(session).DescribeBigInventoryModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new BigInventoryModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
