...
{
    public sealed class InventoryModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public InventoryModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "InventoryModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InventoryRestClient(session).DescribeInventoryModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new InventoryModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
