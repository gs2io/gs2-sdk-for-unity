...
{
    public sealed class BigItemModelHolder : AbstractTreeViewItem
    {
        private BigInventoryModel _parent;

        public BigItemModelHolder(
                int id,
                BigInventoryModel parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "BigItemModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InventoryRestClient(session).DescribeBigItemModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new BigItemModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
