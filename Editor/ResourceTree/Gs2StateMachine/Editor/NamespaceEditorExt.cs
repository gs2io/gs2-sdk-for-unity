...
namespace Gs2.Editor.ResourceTree.Gs2StateMachine.Editor
{
    ...
    {
        public static void OnGUI(Gs2.Gs2StateMachine.Model.Namespace item) {
            ...
            if (item.StartScript == null) {
                ...
                EditorGUILayout.LabelField("StartScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.StartScript);
                EditorGUI.indentLevel--;
            }
            if (item.PassScript == null) {
                EditorGUILayout.TextField("PassScript", "");
            }
            else {
                EditorGUILayout.LabelField("PassScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.PassScript);
                EditorGUI.indentLevel--;
            }
            if (item.ErrorScript == null) {
                EditorGUILayout.TextField("ErrorScript", "");
            }
            else {
                EditorGUILayout.LabelField("ErrorScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.ErrorScript);
                EditorGUI.indentLevel--;
            }
            EditorGUILayout.TextField("LowestStateMachineVersion", UnixTime.FromUnixTime(item.LowestStateMachineVersion ?? 0).ToString(CultureInfo.CurrentUICulture));
