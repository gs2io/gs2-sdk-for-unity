...
{
    public sealed class GlobalMessageHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public GlobalMessageHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "GlobalMessage";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2InboxRestClient(session).DescribeGlobalMessagesFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new GlobalMessage(++id, this._parent, v)
            ).ToList<TreeViewItem>();
