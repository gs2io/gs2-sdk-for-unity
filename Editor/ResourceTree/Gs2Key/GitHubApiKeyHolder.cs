...
{
    public sealed class GitHubApiKeyHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public GitHubApiKeyHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "GitHubApiKey";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2KeyRestClient(session).DescribeGitHubApiKeysFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new GitHubApiKey(++id, this._parent, v)
            ).ToList<TreeViewItem>();
