...
{
    public sealed class GitHubApiKey : AbstractTreeViewItem
    {
        private Gs2.Gs2Key.Model.GitHubApiKey _item;
        private Namespace _parent;
        public string NamespaceName => _parent.NamespaceName;
        public string ApiKeyName => _item.Name;

        public GitHubApiKey(
                int id,
                Namespace parent,
                Gs2.Gs2Key.Model.GitHubApiKey item
        ) {
            this.id = id = id * 100;
            this.depth = 4;
        ...
        public override ScriptableObject ToScriptableObject() {
            ...
            var instance = Gs2.Unity.Gs2Key.ScriptableObject.GitHubApiKey.New(
                parent,
                this._item.Name
            );
            instance.name = this._item.Name + "GitHubApiKey";
            return instance;
        }

        public override void OnGUI() {
            GitHubApiKeyEditorExt.OnGUI(this._item);
            
            if (GUILayout.Button("Create Reference Object")) {
                var directory = "Assets/Gs2/Resources";
                directory += "/Namespace" + "/" + NamespaceName;
                directory += "/GitHubApiKey" + "/" + ApiKeyName;
