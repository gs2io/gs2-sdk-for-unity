...
{
    public sealed class IncrementalRateModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public IncrementalRateModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "IncrementalRateModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2ExchangeRestClient(session).DescribeIncrementalRateModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new IncrementalRateModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
