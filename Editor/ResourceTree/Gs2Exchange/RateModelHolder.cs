...
{
    public sealed class RateModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public RateModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "RateModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2ExchangeRestClient(session).DescribeRateModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new RateModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
