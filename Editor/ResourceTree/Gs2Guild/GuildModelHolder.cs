...
{
    public sealed class GuildModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public GuildModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "GuildModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2GuildRestClient(session).DescribeGuildModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new GuildModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
