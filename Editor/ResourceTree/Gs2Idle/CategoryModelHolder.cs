...
{
    public sealed class CategoryModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public CategoryModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "CategoryModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2IdleRestClient(session).DescribeCategoryModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new CategoryModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
