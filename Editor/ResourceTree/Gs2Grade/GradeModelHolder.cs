...
{
    public sealed class GradeModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public GradeModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "GradeModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2GradeRestClient(session).DescribeGradeModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new GradeModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
