...
{
    public sealed class OutputHolder : AbstractTreeViewItem
    {
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2NewsRestClient(session).DescribeOutputsFuture(
                new DescribeOutputsRequest()
                        .WithNamespaceName(this._parent.NamespaceName)
                        .WithUploadToken(this._parent.UploadToken)
            );
            yield return future;
            ...
            this.children = future.Result.Items.Select(
                v => new Output(++id, this._parent, v)
            ).ToList<TreeViewItem>();
