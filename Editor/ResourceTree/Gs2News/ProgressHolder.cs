...
{
    public sealed class ProgressHolder : AbstractTreeViewItem
    {
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2NewsRestClient(session).DescribeProgressesFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new Progress(++id, this._parent, v)
            ).ToList<TreeViewItem>();
