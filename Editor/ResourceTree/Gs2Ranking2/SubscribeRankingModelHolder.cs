...
{
    public sealed class SubscribeRankingModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public SubscribeRankingModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "SubscribeRankingModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Ranking2RestClient(session).DescribeSubscribeRankingModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new SubscribeRankingModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
