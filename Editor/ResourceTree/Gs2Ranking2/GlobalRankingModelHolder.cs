...
{
    public sealed class GlobalRankingModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public GlobalRankingModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "GlobalRankingModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Ranking2RestClient(session).DescribeGlobalRankingModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new GlobalRankingModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
