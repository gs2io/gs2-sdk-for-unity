...
{
    public sealed class ClusterRankingModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public ClusterRankingModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "ClusterRankingModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2Ranking2RestClient(session).DescribeClusterRankingModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new ClusterRankingModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
