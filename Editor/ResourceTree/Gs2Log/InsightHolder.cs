...
{
    public sealed class InsightHolder : AbstractTreeViewItem
    {
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2LogRestClient(session).DescribeInsightsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new Insight(++id, this._parent, v)
            ).ToList<TreeViewItem>();
