...
namespace Gs2.Editor.ResourceTree.Gs2Log
{
    ...
    {
        private Gs2.Gs2Log.Model.Namespace _item;
        public string NamespaceName => _item.Name;

        public Namespace(
                int id,
                Gs2.Gs2Log.Model.Namespace item
        ) {
            this.id = id = id * 100;
            this.depth = 2;
            ...
            this.children = new TreeViewItem[] {
                new InsightHolder(++id, this)
            }.ToList();
            this._item = item;
        }

        public override ScriptableObject ToScriptableObject() {
            var instance = Gs2.Unity.Gs2Log.ScriptableObject.Namespace.New(
                this._item.Name
            );
