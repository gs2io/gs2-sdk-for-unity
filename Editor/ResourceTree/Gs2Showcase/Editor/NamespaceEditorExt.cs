...
namespace Gs2.Editor.ResourceTree.Gs2Showcase.Editor
{
    ...
    {
        public static void OnGUI(Gs2.Gs2Showcase.Model.Namespace item) {
            ...
            if (item.BuyScript == null) {
                EditorGUILayout.TextField("BuyScript", "");
            }
            else {
                EditorGUILayout.LabelField("BuyScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.BuyScript);
                EditorGUI.indentLevel--;
            }
