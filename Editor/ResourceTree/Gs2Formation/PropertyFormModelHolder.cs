...
{
    public sealed class PropertyFormModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public PropertyFormModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "PropertyFormModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2FormationRestClient(session).DescribePropertyFormModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new PropertyFormModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
