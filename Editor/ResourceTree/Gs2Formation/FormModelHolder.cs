...
{
    public sealed class FormModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public FormModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "FormModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2FormationRestClient(session).DescribeFormModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new FormModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
