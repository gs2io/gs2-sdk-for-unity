...
namespace Gs2.Editor.ResourceTree.Gs2Formation.Editor
{
    ...
    {
        public static void OnGUI(Gs2.Gs2Formation.Model.Namespace item) {
            ...
            if (item.UpdateMoldScript == null) {
                EditorGUILayout.TextField("UpdateMoldScript", "");
            }
            else {
                EditorGUILayout.LabelField("UpdateMoldScript");
                EditorGUI.indentLevel++;
                ScriptSettingEditorExt.OnGUI(item.UpdateMoldScript);
                EditorGUI.indentLevel--;
            }
            if (item.UpdateFormScript == null) {
                EditorGUILayout.TextField("UpdateFormScript", "");
            }
            else {
                EditorGUILayout.LabelField("UpdateFormScript");
                EditorGUI.indentLevel++;
