...
{
    public sealed class MoldModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public MoldModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "MoldModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2FormationRestClient(session).DescribeMoldModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new MoldModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
