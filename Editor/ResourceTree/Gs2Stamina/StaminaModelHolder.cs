...
{
    public sealed class StaminaModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public StaminaModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "StaminaModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2StaminaRestClient(session).DescribeStaminaModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new StaminaModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
