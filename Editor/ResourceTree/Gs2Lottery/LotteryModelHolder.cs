...
{
    public sealed class LotteryModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public LotteryModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "LotteryModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2LotteryRestClient(session).DescribeLotteryModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new LotteryModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
