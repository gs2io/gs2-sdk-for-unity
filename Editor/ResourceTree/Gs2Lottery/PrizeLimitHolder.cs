...
{
    public sealed class PrizeLimitHolder : AbstractTreeViewItem
    {
        private PrizeTable _parent;

        public PrizeLimitHolder(
                int id,
                PrizeTable parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "PrizeLimit";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2LotteryRestClient(session).DescribePrizeLimitsFuture(
                new DescribePrizeLimitsRequest()
                        .WithNamespaceName(this._parent.NamespaceName)
                        .WithPrizeTableName(this._parent.PrizeTableName)
            );
            yield return future;
            ...
            this.children = future.Result.Items.Select(
                v => new PrizeLimit(++id, this._parent, v)
            ).ToList<TreeViewItem>();
