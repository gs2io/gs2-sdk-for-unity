...
{
    public sealed class PrizeTableHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public PrizeTableHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "PrizeTable";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2LotteryRestClient(session).DescribePrizeTablesFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new PrizeTable(++id, this._parent, v)
            ).ToList<TreeViewItem>();
