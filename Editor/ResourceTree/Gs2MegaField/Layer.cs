...
{
    public sealed class Layer : AbstractTreeViewItem
    {
        private Gs2.Gs2MegaField.Model.Layer _item;
        private Namespace _parent;
        public string NamespaceName => _parent.NamespaceName;
        public string AreaModelName => _item.AreaModelName;
        public string LayerModelName => _item.LayerModelName;

        public Layer(
                int id,
                Namespace parent,
                Gs2.Gs2MegaField.Model.Layer item
        ) {
            this.id = id = id * 100;
            this.depth = 4;
            this.icon = EditorGUIUtility.ObjectContent(null, typeof(GameObject)).image.ToTexture2D();
            this.displayName = item.AreaModelName + item.LayerModelName;
        ...
        public override ScriptableObject ToScriptableObject() {
            ...
            var instance = Gs2.Unity.Gs2MegaField.ScriptableObject.Layer.New(
                parent,
                this._item.AreaModelName,
                this._item.LayerModelName
            );
            instance.name = this._item.AreaModelName +this._item.LayerModelName + "Layer";
            return instance;
        }
