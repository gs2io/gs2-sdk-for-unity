...
{
    public sealed class LayerModelHolder : AbstractTreeViewItem
    {
        private AreaModel _parent;

        public LayerModelHolder(
                int id,
                AreaModel parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "LayerModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2MegaFieldRestClient(session).DescribeLayerModelsFuture(
                new DescribeLayerModelsRequest()
                        .WithNamespaceName(this._parent.NamespaceName)
                        .WithAreaModelName(this._parent.AreaModelName)
            );
            yield return future;
            ...
            this.children = future.Result.Items.Select(
                v => new LayerModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
