...
{
    public sealed class AreaModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public AreaModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "AreaModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2MegaFieldRestClient(session).DescribeAreaModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new AreaModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
