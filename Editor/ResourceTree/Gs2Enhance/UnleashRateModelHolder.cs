...
{
    public sealed class UnleashRateModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public UnleashRateModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "UnleashRateModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2EnhanceRestClient(session).DescribeUnleashRateModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new UnleashRateModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
