...
{
    public sealed class QuestGroupModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public QuestGroupModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "QuestGroupModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2QuestRestClient(session).DescribeQuestGroupModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new QuestGroupModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
