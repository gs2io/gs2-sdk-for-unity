...
{
    public sealed class RatingModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public RatingModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "RatingModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2MatchmakingRestClient(session).DescribeRatingModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new RatingModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
