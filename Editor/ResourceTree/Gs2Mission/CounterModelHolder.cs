...
{
    public sealed class CounterModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public CounterModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "CounterModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2MissionRestClient(session).DescribeCounterModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new CounterModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
