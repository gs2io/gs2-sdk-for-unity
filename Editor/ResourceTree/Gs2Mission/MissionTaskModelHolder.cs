...
{
    public sealed class MissionTaskModelHolder : AbstractTreeViewItem
    {
        private MissionGroupModel _parent;

        public MissionTaskModelHolder(
                int id,
                MissionGroupModel parent
        ) {
            this.id = id = id * 100;
            this.depth = 5;
            this.displayName = "MissionTaskModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2MissionRestClient(session).DescribeMissionTaskModelsFuture(
                new DescribeMissionTaskModelsRequest()
                        .WithNamespaceName(this._parent.NamespaceName)
                        .WithMissionGroupName(this._parent.MissionGroupName)
            );
            yield return future;
            ...
            this.children = future.Result.Items.Select(
                v => new MissionTaskModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
