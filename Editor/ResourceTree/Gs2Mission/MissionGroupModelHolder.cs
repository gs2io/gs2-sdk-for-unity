...
{
    public sealed class MissionGroupModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public MissionGroupModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "MissionGroupModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2MissionRestClient(session).DescribeMissionGroupModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new MissionGroupModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
