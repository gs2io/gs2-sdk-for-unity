...
{
    public sealed class BonusModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public BonusModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "BonusModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2LoginRewardRestClient(session).DescribeBonusModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new BonusModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
