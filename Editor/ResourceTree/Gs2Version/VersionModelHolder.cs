...
{
    public sealed class VersionModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public VersionModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "VersionModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2VersionRestClient(session).DescribeVersionModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new VersionModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
