...
{
    public sealed class TakeOverTypeModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public TakeOverTypeModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "TakeOverTypeModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2AccountRestClient(session).DescribeTakeOverTypeModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new TakeOverTypeModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
