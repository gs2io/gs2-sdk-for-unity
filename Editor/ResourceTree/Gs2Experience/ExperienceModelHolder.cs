...
{
    public sealed class ExperienceModelHolder : AbstractTreeViewItem
    {
        private Namespace _parent;

        public ExperienceModelHolder(
                int id,
                Namespace parent
        ) {
            this.id = id = id * 100;
            this.depth = 3;
            this.displayName = "ExperienceModel";
        ...
        public override IEnumerator Load(TreeView view, Gs2RestSession session, Func<bool> reload) {
            var future = new Gs2ExperienceRestClient(session).DescribeExperienceModelsFuture(
            ...
            this.children = future.Result.Items.Select(
                v => new ExperienceModel(++id, this._parent, v)
            ).ToList<TreeViewItem>();
